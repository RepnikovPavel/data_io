#!/bin/bash
# Usage: ./scripts/clean_scibench.sh [INPUT_DIR] [OUTPUT_DIR] [WORKERS]
set -e

INPUT_DIR="${1:-/mnt/hdd2/datasets_text/Platypus/scibench/dataset/original}"
...
docker rm -f hrm_text_clean_scibench 2>/dev/null || true
...
docker run --rm \
  ...
  python -u pipe/clean_platypus/clean_scibench.py \
    --input_dir "$INPUT_DIR" \
    --output_path "$OUTPUT_DIR/scibench.jsonl" \
    --workers "$WORKERS"
