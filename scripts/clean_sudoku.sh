#!/bin/bash
# Usage: ./scripts/clean_sudoku.sh [DATASETS_DIR] [OUTPUT_DIR] [WORKERS]
set -e

DATASETS_DIR="${1:-/mnt/hdd2/datasets_text}"
OUTPUT_DIR="${2:-/mnt/hdd2/datasets_text_transformed/HRM-Text/data_clustered/sudoku_extreme}"
WORKERS="${3:-4}"
...
docker rm -f hrm_text_clean_sudoku 2>/dev/null || true
...
docker run --rm \
  ...
  python -u pipe_clustered/clean_sudoku.py \
