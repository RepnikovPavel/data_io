#!/bin/bash
# Usage: ./scripts/clean_math_train.sh [DATASETS_DIR] [OUTPUT_DIR] [WORKERS]
set -e
...
docker rm -f hrm_text_clean_math_train 2>/dev/null || true
...
docker run --rm \
  --name hrm_text_clean_math_train \
  --user $(id -u):$(id -g) \
  ...
  python -u pipe/clean_math_train.py \
    --output_path "$OUTPUT_DIR/math_train.jsonl" \
    --workers "$WORKERS"
