#!/bin/bash
# Usage: ./scripts/clean_omnimath.sh [DATASETS_DIR] [OUTPUT_DIR] [WORKERS]
set -e
...
docker rm -f hrm_text_clean_omnimath 2>/dev/null || true
...
docker run --rm \
  ...
  python -u pipe/clean_omnimath.py \
    --output_path "$OUTPUT_DIR/omnimath.jsonl" \
    --workers "$WORKERS"
