#!/bin/bash
# Usage: ./scripts/clean_numinamath.sh [DATASETS_DIR] [OUTPUT_DIR] [WORKERS]
set -e
...
docker rm -f hrm_text_clean_numinamath 2>/dev/null || true
...
docker run --rm \
  --name hrm_text_clean_numinamath \
  --user $(id -u):$(id -g) \
  ...
  python -u pipe/clean_numinamath.py \
    --output_path "$OUTPUT_DIR/numinamath.jsonl" \
    --workers "$WORKERS"
