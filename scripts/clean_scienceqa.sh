#!/bin/bash
# Usage: ./scripts/clean_scienceqa.sh [DATASETS_DIR] [OUTPUT_DIR] [WORKERS]
set -e
...
docker rm -f hrm_text_clean_scienceqa 2>/dev/null || true
...
docker run --rm \
  ...
  python -u pipe/clean_platypus/clean_scienceqa.py \
