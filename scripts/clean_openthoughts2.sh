#!/bin/bash
# Usage: ./scripts/clean_openthoughts2.sh [DATASETS_DIR] [OUTPUT_DIR] [WORKERS]
set -e

DATASETS_DIR="${1:-/mnt/hdd2/datasets_text}"
OUTPUT_DIR="${2:-/mnt/hdd2/datasets_text_transformed/HRM-Text/data_clustered/openthoughts2}"
WORKERS="${3:-4}"
...
docker rm -f hrm_text_clean_openthoughts2 2>/dev/null || true
...
docker run --rm \
  --name hrm_text_clean_openthoughts2 \
  --user $(id -u):$(id -g) \
  ...
  python -u pipe_clustered/clean_openthoughts2.py \
