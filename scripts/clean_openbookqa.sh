#!/bin/bash
# Usage: ./scripts/clean_openbookqa.sh [DATASETS_DIR] [OUTPUT_DIR] [WORKERS]
set -e
...
docker rm -f hrm_text_clean_openbookqa 2>/dev/null || true
...
docker run --rm \
  --name hrm_text_clean_openbookqa \
  --user $(id -u):$(id -g) \
  ...
  python -u pipe/clean_platypus/clean_openbookqa.py \
    --output_path "$OUTPUT_DIR/openbookqa.jsonl" \
    --workers "$WORKERS"
