#!/bin/bash
# Usage: ./scripts/clean_webinstruct_verified.sh [DATASETS_DIR] [OUTPUT_DIR] [WORKERS]
set -e
...
docker rm -f hrm_text_clean_webinstruct_verified 2>/dev/null || true
...
docker run --rm \
  --name hrm_text_clean_webinstruct_verified \
  --user $(id -u):$(id -g) \
  ...
  python -u pipe/clean_webinstruct_verified.py \
    --output_path "$OUTPUT_DIR/webinstruct_verified.jsonl" \
    --workers "$WORKERS"
