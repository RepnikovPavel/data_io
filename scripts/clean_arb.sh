#!/bin/bash
# Usage: ./scripts/clean_arb.sh [INPUT_DIR] [OUTPUT_DIR] [WORKERS]
set -e

INPUT_DIR="${1:-/mnt/hdd2/datasets_text/Platypus/ARB}"
...
docker rm -f hrm_text_clean_arb 2>/dev/null || true
...
docker run --rm \
  ...
  python -u pipe/clean_platypus/clean_arb.py \
    --input_dir "$INPUT_DIR" \
