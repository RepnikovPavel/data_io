#!/bin/bash
# Usage: ./scripts/clean_no_robots.sh [DATASETS_DIR] [OUTPUT_DIR] [WORKERS]
set -e
...
docker rm -f hrm_text_clean_no_robots 2>/dev/null || true
...
docker run --rm \
  ...
  python -u pipe/clean_no_robots.py \
    --output_path "$OUTPUT_DIR/no_robots.jsonl" \
    --workers "$WORKERS"
