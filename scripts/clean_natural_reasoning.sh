#!/bin/bash
# Usage: ./scripts/clean_natural_reasoning.sh [DATASETS_DIR] [OUTPUT_DIR] [WORKERS]
set -e
...
docker rm -f hrm_text_clean_natural_reasoning 2>/dev/null || true
...
docker run --rm \
  --name hrm_text_clean_natural_reasoning \
  --user $(id -u):$(id -g) \
  ...
  python -u pipe/clean_natural_reasoning.py \
    --output_path "$OUTPUT_DIR/natural_reasoning.jsonl" \
    --workers "$WORKERS"
