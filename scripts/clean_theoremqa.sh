#!/bin/bash
# Usage: ./scripts/clean_theoremqa.sh [DATASETS_DIR] [OUTPUT_DIR] [WORKERS]
set -e
...
docker rm -f hrm_text_clean_theoremqa 2>/dev/null || true
...
docker run --rm \
  ...
  python -u pipe/clean_platypus/clean_theoremqa.py \
    --output_path "$OUTPUT_DIR/theoremqa.jsonl" \
    --workers "$WORKERS"
