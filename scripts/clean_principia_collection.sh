#!/bin/bash
# Usage: ./scripts/clean_principia_collection.sh [DATASETS_DIR] [OUTPUT_DIR] [WORKERS]
set -e
...
docker rm -f hrm_text_clean_principia_collection 2>/dev/null || true
...
docker run --rm \
  --name hrm_text_clean_principia_collection \
  --user $(id -u):$(id -g) \
  ...
  python -u pipe/clean_principia_collection.py \
    --output_path "$OUTPUT_DIR/principia_collection.jsonl" \
    --workers "$WORKERS"
